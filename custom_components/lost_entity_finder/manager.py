...
from .const import (
    DEBOUNCE_COOLDOWN,
    DOMAIN,
    REGISTRY_RESCAN_DOMAINS,
    RESCAN_COMPONENT_DOMAINS,
    TRANSLATION_KEY_LOST,
)
...
class EntityFinderManager:
    ...
    async def async_setup(self) -> None:
        ...
        for event_type in (EVENT_AUTOMATION_RELOADED, EVENT_SCENE_RELOADED):
            self._unsubs.append(
                self.hass.bus.async_listen(event_type, self._handle_rescan_event)
            )
        self._unsubs.append(
            self.hass.bus.async_listen(
                EVENT_CORE_CONFIG_UPDATE, self._handle_rescan_event
            )
        )
    ...
    def _handle_component_loaded(self, event: Event) -> None:
        ...
        if domain in RESCAN_COMPONENT_DOMAINS:
            self._schedule_scan()
    ...
    def _handle_registry_event(self, event: Event) -> None:
        ...
        if entity_id.partition(".")[0] in REGISTRY_RESCAN_DOMAINS:
            self._schedule_scan()
    ...
    async def async_restore_ignored(self) -> None:
        """Restore previously ignored lost entity repairs."""
        for old_entity_id in self.store.get_ignored_entity_ids():
            ir.async_ignore_issue(
                self.hass,
                DOMAIN,
                slugify_issue_id(old_entity_id),
                False,
            )
